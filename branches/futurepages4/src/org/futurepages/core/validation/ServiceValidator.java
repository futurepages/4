...
public abstract class ServiceValidator<SERVICES extends EntityServices>  {
	...
	private ArrayList<ServiceValidator> subValidators = new ArrayList();

	protected Boolean breakOnFirst;

	protected SERVICES services;

	private static <T extends ServiceValidator,SERVICES extends EntityServices> T validate(Class<T> t, SERVICES services, boolean breakOnFirst) {
		T validator;
		try {
	...
	public static <T extends ServiceValidator, SERVICES extends EntityServices> void validate(SERVICES services, Class<T> clss, Executor<T> executor){
	...
	protected <T extends ServiceValidator> T validate(Class<T> t) {
		T validator = ServiceValidator.validate(t,null, breakOnFirst);
	...
	private HashMap<String, String> validationMapAux() {
		for (ServiceValidator v : subValidators) {
	...
	public interface Executor<V extends ServiceValidator> {

		public abstract void execute(V ServiceValidator);
	}
}