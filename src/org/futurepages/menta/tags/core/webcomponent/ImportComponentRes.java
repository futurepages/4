...
//		System.out.println("[importJS"+(async?" {async}":"")+"] {"+key+">"+version+"}"); // for DEBUG-MODE
		return  concat("<script type=\"text/javascript\" src=\"" , resPath(req) , "/" , key , "/" , version , "/" , key , ".js"+ Apps.get("RELEASE_QUERY")+"\"></script>"
	...
	private String importCSS(HttpServletRequest req, boolean async){
		if(!async){
			return concat("<link rel=\"stylesheet\" type=\"text/css\" href=\"" , resPath(req) , "/" , key , "/" , version , "/" , key , ".css"+Apps.get("RELEASE_QUERY")+"\" media=\"all\"/>");
		}else{
			String hasModule = !Is.empty(moduleId) ? concat(",'", moduleId, "'") : "";
